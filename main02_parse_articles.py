...
def parallel_parse(pl, nlp_eng):
    ...
    if pl.batch_range is not None:
        ...
        print (pl.num_articles//batchsize)
        ...
        for art_nlp, art_meta in nlp_eng.pipe(art_data, as_tuples=True, n_process=1):
        #for art_nlp, art_meta in nlp_eng.pipe(art_data, as_tuples=True):
            #print (art_nlp)
            contract_id = art_meta["contract_id"]
            art_num = art_meta["article_num"]
...
def serial_parse(pl, nlp_eng):
    ...
    art_list = pl.stream_art_data(start=0, end=pl.num_articles, verbose=True)
    for art_num, cur_art in enumerate(art_list):
        #print (art_num)
        #print (cur_art)
        
        art_nlp, art_meta = cur_art
        contract_id = art_meta["contract_id"]
        art_num = art_meta["article_num"]
        art_nlp = nlp_eng(art_nlp)	
        art_statements = get_statements(art_nlp,contract_id, art_num)
        statement_list.extend(art_statements)
    return statement_list


def parse_articles(pl, parallel=False):
