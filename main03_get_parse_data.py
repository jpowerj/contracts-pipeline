...
def extract_pdata(pl, pdata_path):
    ...
    #    num_to_process = pl.batch_range + 1
    #else:
    ...
    if pl.batch_range is not None:
        ...
        print (num_to_process//batchsize)
        #batched_filenames = ["02_canadian_parsed_%d.pkl" % i for i in range(pl.num_articles//batchsize+1)]
        if pl.parallel_parsing:
            files = os.listdir(os.path.join(pl.get_output_path(), "02_parsed_articles"))
            batched_filenames = [os.path.join("02_parsed_articles", fn) for fn in files]
        else:
            batched_filenames = ["02_canadian_parsed_%d.pkl" % i for i in range(num_to_process//batchsize+1)]
        print (batched_filenames)
        if all([os.path.exists(i) for i in batched_filenames]):
            pass
        elif os.path.exists(os.path.join(pl.get_output_path(), "02_canadian_parsed.pkl")):
            batched_filenames = [os.path.join(pl.get_output_path(), "02_canadian_parsed.pkl")]
        print (len(batched_filenames))
        #for statement_data in pl.pbar(pl.stream_parses(), total=num_to_process):
        for filename in pl.pbar(batched_filenames, total=len(batched_filenames)):
            for statement_data in pl.stream_parses(parses_fpath=filename):
                contract_id = statement_data["contract_id"]
                #  contract_id 0000102a_eng
                ...
                #for i,j in statement_dict.items():
                #    print (i,j)
                #input("")         
