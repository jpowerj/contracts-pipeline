...
def sum_auth_sections(pl):
    ...
    for chunk_num, cur_chunk_df in pl.stream_statement_auth():
        ...
        summed_chunk_df = sum_auth_df_sections(pl, cur_chunk_df)
    ...
    final_summed_df = summed_df.groupby(["contract_id", "article_num"]).sum()
    #final_summed_df = summed_df.groupby(["contract_id"]).sum()
    final_summed_df.reset_index(inplace=True)
    pl.save_authsums_df(summed_df)

    sum_csv_fpath = pl.get_authsums_fpath_sections(extension="csv")
    final_summed_df.to_csv(sum_csv_fpath, index=False)
    sum_pkl_fpath = pl.get_authsums_fpath_sections(extension="pkl")
    final_summed_df.to_pickle(sum_pkl_fpath)
    pl.iprint("Saved " + str(sum_csv_fpath))

def sum_auth_df_sections(pl, auth_df):
    """
    ...
    unique_ids = ["contract_id","article_num", "subnorm"]
    ...
    #final_summed_df = grp_df.groupby(["contract_id", "article_num"]).sum()
    final_summed_df = grp_df.groupby(["contract_id", "article_num"]).sum()
    #final_summed_df = grp_df.groupby(["contract_id"]).sum()
    final_summed_df.reset_index(inplace=True)
    print (final_summed_df)

    # Return it so these summed chunk dfs can be combined into a final summed df
    # by sum_auth()
    return final_summed_df
