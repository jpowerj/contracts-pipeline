...
def compute_word_counts(pl):
    ...
    #auth_df = [os.path.join("output_all_data/04_auth/", fn) for fn in os.listdir("output_all_data/04_auth/")]
    #auth_df = "../output_all_data/04_canadian_auth.pkl"
    auth_df = os.path.join(pl.get_output_path(), "04_" + pl.corpus_name + "_auth.pkl")
    print (auth_df)
    """
    input("")
    these are the wrong dfs, implemented the following function in pipeline_canadian though: get_latest_auth_df()
    #sum_csv_fpath = pl.get_authsums_fpath(extension="csv")
    #final_summed_df.to_csv(sum_csv_fpath, index=False)
    #sum_pkl_fpath = pl.get_authsums_fpath(extension="pkl")
    #sum_pkl_fpath = pl.get_authsums_fpath(extension="pkl")
    """
    auth_df = pd.read_pickle(auth_df)
    ...
    col_list = auth_df.columns.tolist()
    vars_to_keep = [v for v in vars_to_keep if v in col_list]
    ...
    #agg_dict = set([i for i in agg_dict if i in wc_groups.columns.tolist()])
    #agg_dict = set([i for i in agg_dict if i in wc_df.columns.tolist()])
