...
##############################
    ...
    def get_plaintext_fpaths(self):
        ...
        all_fpaths = []
        print (plaintext_path, self.corp_extensions)
        ...
        print("all_fpaths: ", len(all_fpaths))
        self.lang_list = []
        ...
        print("all_fpaths: ", len(lang_fpaths))
    ...
    def stream_parses(self, parses_fpath=None):
        ...
        if parses_fpath is None:
            ...
            parses_fpath = os.path.join(self.get_output_path(), parses_fpath)
        """
        else:
            parses_list = []
            for filename in parses_fpath[-1:]:
                filename = os.path.join(self.get_output_path(), filename)
                parses_list.extend(joblib.load(filename))
                print (len(parses_list))
                for i in parses_list:
                    print (i)
                    input("")
                sys.exit(0)
        """
    ...
    def stream_pt_contract_data(self, start=0, end=None, pt_only=True):
        """
        ...
        Otherwise (the default), you get a stream of Contract objects. (TODO)
        """
        if pt_only:

            print ("streaming pt data from ",self.get_plaintext_fpaths()[0], len(self.get_plaintext_fpaths()))
            for cur_doc in self.get_plaintext_fpaths():
                cur_id = os.path.basename(cur_doc)[:-4]
                cur_lang = cur_id.split("_")[-1].split(".")[0]
                with open(cur_doc) as f:
                    cur_text = f.read().strip()
                cur_contract_data = {'contract_id':cur_id, 'lang':cur_lang, 'text':cur_text}
                yield cur_contract_data
        """
        self.dprint("running stream_pt_contracts()")
        client = MongoClient()
        db = client["contracts"]
        collection = db[self.get_corpus_name()]
    ...
    def stream_statement_auth(self):
        ...
        stauth_path = self.get_statement_auth_path()
        cname = self.get_corpus_name()
        #fpaths = [(n,os.path.join(stauth_path,f"04_{cname}_auth_{n}.pkl")) for n in range(self.num_chunks)]
        #fpaths = [(n,os.path.join(stauth_path,f"04_{cname}_auth_{n}.pkl")) for n in range(min(self.num_chunks, len(os.listdir(stauth_path))))]
        fpaths = [(n,os.path.join(stauth_path,f"04_{cname}_auth_{n}.pkl")) for n in range(len(os.listdir(stauth_path)))]
        #fpaths = [(1,os.path.join(stauth_path,f"04_{cname}_auth.pkl"))]
...
# to just *return* instance variables)
    ...
    def get_meta_fpath(self):
        return os.path.join(self.corpus_path, self.get_corpus_name() + "_meta.csv")
    ### Logs
    ...
    def get_auth_fpath(self):
        return os.path.join(self.get_output_path(), f"04_{self.get_corpus_name()}_auth.pkl")

    """
    def get_latest_auth_df(self):
        auth_path = self.get_statement_auth_path()
        fpath_data = plu.sort_by_suffix(auth_path)
        new_fname = plu.remove_suffix(auth_path)
        return os.path.join(self.get_output_path(), new_fname)
    """
    ...
    #def get_statement_auth_fpath(self):
    #    return f"04_{self.get_corpus_name()}_auth.pkl"
    
    def get_authsums_fpath(self, extension="csv"):
        return os.path.join(self.get_output_path(),"05_" + self.get_corpus_name()
                            + "_authsums." + extension)

    def get_authsums_fpath_sections(self, extension="csv"):
        return os.path.join(self.get_output_path(),"05_" + self.get_corpus_name()
                            + "_authsums_sections." + extension)

    def save_authsums_df(self, authsums_df):
        authsums_fpath = os.path.join(self.get_output_path(),
                                      f"05_{self.get_corpus_name()}_authsums_all_articles.pkl")
...
#######################
    def __init__(self, corpus_name, corpus_path=None, data_path=None,
                 output_path=None, use_sqlite=True, use_mongo=False, num_chunks=20,
                 default_subnorms=False, ocr=OCR_OPTIONS[0], parser=PARSER_OPTIONS[0],
                 num_topics=20, lang_list=["eng"], corp_extensions=["txt"],
                 debug=True, in_memory=False, default_encoding="utf-8", id_list_fpath=None,
                 ...
                 break_id=None, recurse_corpus_dir=True, split_method="parser", num_articles=None, remove_top_n=200, parallel_parsing=False):
        """
        ...
        # First things first: chdir so that this file is in the working dir
        ...
        # remove the top n most frequent words before running LDA
        ...
        print (os.path.join(self.output_path, "01_artsplit"))
        self.num_articles = len(os.listdir(os.path.join(self.output_path, "01_artsplit")))
        ...
        print ("subnorm list", self.subnorm_list)
        #input("")
...
############################
    ...
    def run_main(self, steps=None):
        ...
        for cur_step in steps:
            ...
            # cur_step_name[3:] doesn't work, e.g. "04b_combine_auth" has [4:], thus replaced
            mod_fn = getattr(step_module, "_".join(cur_step_name.split("_")[1:])) # cur_step_name[3:])
            # Now we call it with the specific kwargs
            print ("here", cur_step_kwargs, self.get_artsplit_path())
    ...
    def run_lda(self, steps=None):
        ...
        """
        from lda09_save_lda_topics import save_lda_topics
        from lda10_compute_weights import compute_weights
        from lda11_construct_subnorm_auths import construct_subnorm_auths
        from lda12_weight_auth_measures import weight_auth_measures
        from lda13_sum_weighted_auths import sum_weighted_auths
        from lda14_generate_auth_graphs import generate_auth_graphs

        """
	# too bad, they seem to not be in sync
	labor-contracts/lda_pipeline/lda01_extract_object_branches.py
	labor-contracts/lda_pipeline/lda02_construct_dictionary.py
	labor-contracts/lda_pipeline/lda03_split_docs.py
	labor-contracts/lda_pipeline/lda04_export_to_aws.py
	labor-contracts/lda_pipeline/lda05_remote_control_aws.py
	labor-contracts/lda_pipeline/lda06_construct_corpus.py
	labor-contracts/lda_pipeline/lda07_combine_corpora.py
	labor-contracts/lda_pipeline/lda08_run_lda.py
	labor-contracts/lda_pipeline/lda09_save_lda_topics.py
	labor-contracts/lda_pipeline/lda10_compute_weights.py
	labor-contracts/lda_pipeline/lda11_construct_subnorm_auths.py
	labor-contracts/lda_pipeline/lda12_weight_auth_measures.py
	labor-contracts/lda_pipeline/lda13_sum_weighted_auths.py
	labor-contracts/lda_pipeline/lda14_generate_auth_graphs.py
	labor-contracts/lda_pipeline/ldasplit10_construct_subnorm_auths.py
	labor-contracts/lda_pipeline/ldasplit11_weight_subnorm_auths.py
	labor-contracts/lda_pipeline/ldasplit12_combine_subnorm_auths.py
        """
        ...
        self.lda_fns["save_lda_topics"] = save_lda_topics
...
if __name__ =="__main__":
    pipeline = Pipeline("canadian", force_overwrite=True, batch_range=2000, parallel_parsing=False)
    #steps= ["01_split_contracts", "02_parse_articles", "03_get_parse_data", "04_compute_auth", "04b_combine_auth", "05_sum_auth" ,"05b_sum_auth_sections", "06_summary_stats", "07_merge_auth_meta"]
    steps= ["02_parse_articles", "03_get_parse_data", "04_compute_auth", "04b_combine_auth", "05_sum_auth" ,"05b_sum_auth_sections", "06_summary_stats", "07_merge_auth_meta"]


    pipeline.run_main(steps=steps)
